...
const FooterLinks = ({heading, items}) => (
  <div className="flex-1 justify-start items-start">
    ...
    {
      items.map((item,idx) => (
        <p key={idx} className="font-poppins dark:text-white text-nft-black-1 font-normal text-base cursor-pointer dark:hover:text-nft-gray-1 hover:text-nft-black-1 my-3">{item}</p>
      ))
    }
  </div>
)
...
const Footer = () => {
  ...
  return (
    <footer className="flexCenter flex-col border-t dark:border-nft-black-1 border-nft-gray-1 sm:py-8 py-16">
       ...
       <div className="flex flex-row w-full mt-5 minmd:w-4/5 md:flex-col sm:px-4 px-16">
       ...
       <div className="flexCenter w-full mt-5 border-t dark:border-nft-black-1 border-nft-gray-1 sm:px-4 px-16">
        <div className="flexBetween flex-row w-full minmd:w-4/5 sm:flex-col mt-7">
          <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text-base">Nestio, Inc. All Rights Reserved.</p>
        <div className="flex flex-row sm:mt-4">
            {[images.instagram, images.twitter, images.telegram, images.discord].map( (image, idx) => (
              <div key={idx} className="mx-2 cursor-pointer">
                <Image src={image} className={`object-contain ${theme === "light" ? "filter invert" : ""}`} width={24} height={24} alt="social"/>
              </div>
            ))}
        </div>
        </div>
